    ...
    <section class="section">
        <div class="container">
            <div class="row">
                ...
                {{--
                <x-fronted.services :$service :$index /> --}}
                <div class="icon-box-item text-center col-lg-4 col-md-6 mb-4">
                    <div class="rounded shadow py-5 px-4">
                         <img style="max-width: 75px; border-radius: 10px;" class="" src="{{ asset('storage/' . $service->icon_class ) }}" alt="">
                        <h3 class="mb-3"> {{ $service->title }} </h3>
                        <p class="mb-4"> {{ $service->short_desc }}</p>
                        <a wire:navigate.hover class="btn btn-sm btn-outline-primary" href="{{ route('fronted.single.service', $service) }}">View Details <i
